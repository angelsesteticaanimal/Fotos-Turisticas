...
function updateZoomLabel(){const el=$('zoomValue');if(el)el.textContent=Math.round((+zoomEl.value||1)*100)+'%';}
function setZoom(value){zoomEl.value=Math.max(+zoomEl.min||0.1,Math.min(+zoomEl.max||6,value));updateZoomLabel();draw();}
...
$('zoomOutBtn').onclick=()=>setZoom((+zoomEl.value||1)/1.12);
$('zoomInBtn').onclick=()=>setZoom((+zoomEl.value||1)*1.12);
...
canvas.addEventListener('pointerdown',e=>{const rect=canvas.getBoundingClientRect();const cx=(e.clientX-rect.left)*canvas.width/rect.width,cy=(e.clientY-rect.top)*canvas.height/rect.height;if(pickingColor&&photo){const px=ctx.getImageData(Math.round(cx),Math.round(cy),1,1).data;$('chromaColor').value='#'+[px[0],px[1],px[2]].map(v=>v.toString(16).padStart(2,'0')).join('');pickingColor=false;$('pickColorBtn').textContent='🎯 Tocar na cor do fundo';invalidate();autoFitPerson();return;}if(refineEnabled&&mode!=='normal'){painting=true;paintRefineAt(cx,cy);}else{dragging=true;lastX=e.offsetX;lastY=e.offsetY;}canvas.setPointerCapture(e.pointerId);});
canvas.addEventListener('pointermove',e=>{if(!photo)return;const rect=canvas.getBoundingClientRect();const cx=(e.clientX-rect.left)*canvas.width/rect.width,cy=(e.clientY-rect.top)*canvas.height/rect.height;if(painting&&refineEnabled){paintRefineAt(cx,cy);return;}if(!dragging)return;const sx=canvas.width/canvas.clientWidth,sy=canvas.height/canvas.clientHeight;offsetX+=(e.offsetX-lastX)*sx;offsetY+=(e.offsetY-lastY)*sy;lastX=e.offsetX;lastY=e.offsetY;draw();});
canvas.addEventListener('pointerup',()=>{dragging=false;painting=false;});canvas.addEventListener('pointercancel',()=>{dragging=false;painting=false;});
...
canvas.addEventListener('touchend',e=>{if(e.touches.length<2)pinchStartDistance=0;},{passive:true});
...
$('saveBtn').onclick=async()=>{const blob=await makeBlob(); await uploadFinalToServer(blob); const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`foto-turismo-v323-${String(Date.now()).slice(-6)}.jpg`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);};
$('shareBtn').onclick=async()=>{const blob=await makeBlob(); await uploadFinalToServer(blob); const file=new File([blob],'foto-turismo-v323.jpg',{type:'image/jpeg'});if(navigator.canShare?.({files:[file]}))await navigator.share({title:'Foto Turismo',text:'Minha foto turística',files:[file]});else alert('Use o botão Salvar foto neste navegador.');};
...
// V3.2.3: service worker removido para evitar cache antigo no GitHub Pages.
